...
export const createProduct = asyncHandler(async (req, res, next) => {
    ...
    for (const file of req.files) {
        try {
            ...
            console.log(req.user, "User ID from token:", req.user.id);
...
export const getProducts = asyncHandler(async (req, res, next) => {
    try {
        ...
        let successRes = createSuccess(200, "Products retrieved successfully");
        res.status(200).json({ successRes, data: products });
    } catch (error) {
        next(error)
    }
});
